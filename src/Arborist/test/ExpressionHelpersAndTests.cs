...
public class ExpressionHelpersAndTests {
    [Fact]
    public void And_should_return_true_when_empty() {
        var expr = ExpressionHelpers.And(Enumerable.Empty<Expression<Func<string, bool>>>());
    ...
    public void And_should_work_with_1_expression() {
        var expr = ExpressionHelpers.And<Func<string, bool>>(x => true);
    ...
    public void And_should_work_with_2_expressions() {
        var expr = ExpressionHelpers.And<Func<string, bool>>(x => true, x => false);
    ...
    public void And_should_work_with_3_expressions() {
        var expr = ExpressionHelpers.And<Func<string, bool>>(x => true, x => false, x => true);
